...
def create_test_tar_with_prefix(tmp_path: Path) -> Path:
    """Create a test tar.gz archive with a top-level directory."""
    archive_path = tmp_path / "test.tar.gz"
    with tarfile.open(archive_path, "w:gz") as tar:
        # Add a file
        file_path = tmp_path / "test.txt"
        file_path.write_text("test content")
        tar.add(file_path, arcname="test_prefix/test.txt")
        # Add a directory
        dir_path = tmp_path / "test_dir"
        dir_path.mkdir()
        (dir_path / "subfile.txt").write_text("sub content")
        tar.add(dir_path, arcname="test_prefix/test_dir")
    return archive_path
...
def test_extract_tar_strip_prefix(tmp_path):
    """Test extracting a tar.gz archive with a top-level directory."""
    archive_path = create_test_tar_with_prefix(tmp_path)
    extract_to = tmp_path / "extract"
